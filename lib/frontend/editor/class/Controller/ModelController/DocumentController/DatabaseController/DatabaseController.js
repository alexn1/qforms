...
DatabaseController.prototype.actionNewParam = function() {
    var self = this;
    Param.prototype.getView2('new.html').then(function (result) {
        $(document.body).append(result.view);
        $('#myModal').on('hidden.bs.modal', function(e){$(this).remove();});
        $("#myModal button[name='create']").click(function() {
            var paramName = $("#myModal input[id='paramName']").val();
            self.model.newParam2(paramName).then(function (paramData) {
...
DatabaseController.prototype.createTab = function(docs) {
    ...
    self.model.getView2('DatabaseView/DatabaseView.html').then(function (result) {
        var $div = $(result.view);
...
DatabaseController.prototype.onTableSelect = function(e) {
    ...
    self.model.getTableInfo2(self.tableName).then(function (data) {
...
DatabaseController.prototype.createForm = function() {
    var self = this;
    self.model.getView2('newForm.ejs').then(function (result) {
        ...
        $("#modal button[name='create']").click(function() {
            ...
            if (pageItem.ctrl instanceof PageLinkController) {
                self.applicationController.editorController.pageLinkToPage2(pageItem).then(todo);
            } else {
                todo();
            }
            */
            Promise.try(function () {
                if (pageItem.ctrl instanceof PageLinkController) {
                    return self.applicationController.editorController.pageLinkToPage2(pageItem);
                }
            }).then(function () {
                var params = formWizard.getFormParams();
