...
GridColumn.prototype.init = function() {
    var self = this;
    self.viewClass  = $(self.headerCell).data('view-class');        // name of view css class
    self.initResize(self.headerCell, self.name);
};
...
GridColumn.prototype.renderView = function () {
    var self = this;
    var $view = $('<div><span></span></div>');
    if (self.viewClass) {
        $view.addClass(self.viewClass);
    }
    return $view.get(0);
};
...
GridColumn.prototype.unselected = function(bodyCell) {
    ...
    if (bodyCell.isEdited) {
        ...
        self.restoreValue(bodyCell);
        self.endEdit(bodyCell);
    }
};
...
GridColumn.prototype.enterPress = function(bodyCell) {
    var self = this;
    if (bodyCell.isEdited) {
        if (bodyCell.oldValue !== self.getValue(bodyCell.firstElementChild)) {
            self.gridWidget.save(bodyCell);
        }
        self.endEdit(bodyCell);
...
GridColumn.prototype.beginEdit = function(bodyCell) {
    ...
    bodyCell.oldValue = self.getValue(bodyCell.firstElementChild);
};
...
GridColumn.prototype.restoreValue = function(bodyCell) {
    var self = this;
    self.setValue(bodyCell.firstElementChild, bodyCell.oldValue);
};