...
DataSource.prototype.setValue = function(row, column, value) {
    ...
    if (self.data.table !== '') {
        ...
        //self.eventChanged.fire({source: self});
...
DataSource.prototype.onTableUpdated = function(ea) {
    ...
    self._refresh2().then(function () {
        //self.eventUpdated.fire({source: self});
...
DataSource.prototype.onTableInsert = function(e) {
    ...
    self._refresh2().then(function () {
        if (self.rowsByKey[e.key]) {
            //var _e = new QForms.EventArg(self);
            //_e.key = e.key;
            //self.eventInsert.fire({source: self, key: e.key});
...
DataSource.prototype.frame = function(params, frame) {
    ...
    return Promise.try(function () {
        ...
        self._getData2(params).then(function (data) {
            ...
            _old.rowsByKey = _new.rowsByKey;
            _old.childs    = _new.childs;
            //self.eventNewFrame.fire({source: self});
...
DataSource.prototype.fireRefillRow = function(key, i) {
    ...
    //ea.key = key;
    //ea.i = i;
    //self.eventRefillRow.fire({source: self, key: key, i: i});
...
DataSource.prototype.fireRemoveRow = function(key) {
    ...
    //var ea = new QForms.EventArg(self);
    //ea.key = key;
    //self.eventRemoveRow.fire({source: self, key: key});
...
DataSource.prototype.fireNewRow = function(i, parentKey, key) {
    ...
    //ea.key       = key;
    //self.eventNewRow.fire({source: self, i: i, parentKey: parentKey, key: key});
...
DataSource.prototype.fireMoveRow = function(oldIndex, newIndex, key, parentKey) {
    ...
    //ea.key       = key;
    //ea.parentKey = parentKey;
    //self.eventMoveRow.fire({source: self, oldIndex: oldIndex, newIndex: newIndex, key: key, parentKey: parentKey});
...
DataSource.prototype.fireGoneRow = function(parentKey, key, newParentKey, newIndex) {
    ...
    //ea.newIndex     = newIndex;
    //self.eventGoneRow.fire({source: self, parentKey: parentKey, key: key, newParentKey: newParentKey, newIndex: newIndex});
...
DataSource.prototype.fireComeRow = function(parentKey, key, oldParentKey, newIndex) {
    ...
    //ea.newIndex     = newIndex;
    //self.eventComeRow.fire({source: self, parentKey: parentKey, key: key, oldParentKey: oldParentKey, newIndex: newIndex});
