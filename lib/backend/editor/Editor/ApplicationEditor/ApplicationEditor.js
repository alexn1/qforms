...
ApplicationEditor.prototype.createEjs = function(params, callback) {
    var self = this;
    self.getCustomFilePath('ejs', function(customEjsFilePath) {
        self.createFileByReplace(customEjsFilePath, self.defaultEjsFilePath, self.getViewName(), self.name, null, function(ejs) {
            callback(ejs);
        });
    });
};
