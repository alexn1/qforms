        ...
        if ((self.parent instanceof qforms.Form) && self.name === 'default') {
            return Promise.each(Object.keys(self.parent.fields), function (name) {
                ...
                if (row.hasOwnProperty(columnName)) {
                    if (self.parent instanceof qforms.TableForm) {
    ...
    return DataSource.super_.prototype.fill.call(self, context).then(function (response) {
        ...
        if (self.form && self.name === 'default' && context.newMode) {
            ...
            return self.select(context).then(function (rows) {
                console.log('DataSource.prototype.fill', self.getFullName(), 'select:', Date.now() - start, 'ms');
                response.rows = rows;
                ...
                if (self.data['@attributes'].limit) {
                    ...
                    return self.selectCount(context).then(function (count) {
                        console.log('DataSource.prototype.fill', self.getFullName(),'selectCount:', Date.now() - start, 'ms');
...
DataSource.prototype.frame = function(context) {
    ...
    start = Date.now();
    return self.select(context).then(function (rows) {
        console.log('DataSource.prototype.frame', self.getFullName(), 'select:', Date.now() - start, 'ms');
        return Promise.each(rows, function (row) {
            ...
            return Promise.try(function () {
                if (self.data['@attributes'].limit) {
                    ...
                    start = Date.now();
                    return self.selectCount(context).then(function (count) {
                        console.log('DataSource.prototype.frame', self.getFullName(), 'selectCount:', Date.now() - start, 'ms');
                        response.count = parseInt(count);
                    });
                }
            }).then(function () {
