...
DataSource.prototype.checkColumns = function(row) {
    ...
    if ((self.parent instanceof qforms.Form) && self.name === 'default') {
        for (var name in self.parent.fields) {
            var field = self.parent.fields[name];
            self.checkColumn(row, field.data['@attributes'].column);
        }
    }
};
...
DataSource.prototype.fill = function(context) {
    ...
    return DataSource.super_.prototype.fill.call(self, context).then(function (response) {
        ...
        console.log('DataSource.prototype.fill', self.getFullName(), Date.now() - start);
        return response;
    });
};
