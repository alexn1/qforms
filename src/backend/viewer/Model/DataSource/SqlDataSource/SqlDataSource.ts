...
class SqlDataSource extends DataSource {
    ...
    static addInsertToResult(result, database, table, key) {
        ...
        if (!result[database].insert) result[database].insert = {};
        if (!result[database].insert[table]) result[database].insert[table] = [];
        result[database].insert[table].push(key);
    }

    static addInsertExToResult(result, database, table, key, row) {
        if (!result[database]) result[database] = {};
        if (!result[database].insertEx) result[database].insertEx = {};
        if (!result[database].insertEx[table]) result[database].insertEx[table] = {};
    ...
    static addUpdateToResult(result, database, table, oldKey, newKey) {
        ...
        if (!result[database]) result[database] = {};
        if (!result[database].update) result[database].update = {};
        if (!result[database].update[table]) result[database].update[table] = {};
    ...
    static addUpdateExToResult(result, database, table, oldKey, row) {
        ...
        if (!result[database]) result[database] = {};
        if (!result[database].updateEx) result[database].updateEx = {};
        if (!result[database].updateEx[table]) result[database].updateEx[table] = {};
        result[database].updateEx[table][oldKey] = row;
    }

    static addDeleteToResult(result, database, table, key) {
        if (!result[database]) result[database] = {};
        if (!result[database].delete) result[database].delete = {};
        if (!result[database].delete[table]) result[database].delete[table] = [];
